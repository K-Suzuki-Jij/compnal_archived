...
def test_model_make_ising():
    ...
    ising = model.make_ising(
        lattice=lattice.Triangle(x_size=3, y_size=4), linear=1.1, quadratic=-2.3
    )
    assert isinstance(ising, model.Ising)

    ising = model.make_ising(
        lattice=lattice.Honeycomb(x_size=3, y_size=4), linear=1.1, quadratic=-2.3
    )
    assert isinstance(ising, model.Ising)
    ...
    ising = model.make_ising(
        lattice=lattice.InfiniteRange(system_size=4), linear=1.1, quadratic=-2.3
    )
    assert isinstance(ising, model.Ising)
