...
TEST(LatticeSquare, Constructor) {
   
   EXPECT_EQ((lattice::Square{8, 7}.GetSystemSize()), 56);
   EXPECT_EQ((lattice::Square{8, 7}.GetBoundaryCondition()), lattice::BoundaryCondition::OBC);

   EXPECT_EQ((lattice::Square{
   ...
   EXPECT_THROW((lattice::Square{-1, +9}), std::runtime_error);
   EXPECT_THROW((lattice::Square{+3, -1}), std::runtime_error);
   EXPECT_THROW((lattice::Square{-2, -1}), std::runtime_error);

   EXPECT_THROW((lattice::Square{+2, +2, lattice::BoundaryCondition::NONE}), std::runtime_error);
   EXPECT_THROW((lattice::Square{-2, -2, lattice::BoundaryCondition::NONE}), std::runtime_error);

}

TEST(LatticeSquare, SetSystemSize) {
   
   lattice::Square square{0, 0};
   square.SetXSize(3);
   square.SetYSize(1);
...
TEST(LatticeSquare, SetBoundaryCondition) {
   
   lattice::Square square{0, 0, lattice::BoundaryCondition::PBC};
