...
class Hubbard_1D : public BaseU1Electron_1D<RealType> {
  ...
  public:
   ...
   Hubbard_1D() : BaseU1Electron_1D<RealType>() { onsite_operator_ham_ = CreateOnsiteOperatorHam(h_z_); }
   ...
   Hubbard_1D(const int system_size, const int total_electron)
       : BaseU1Electron_1D<RealType>(system_size, total_electron) {
      onsite_operator_ham_ = CreateOnsiteOperatorHam(h_z_);
   }
   ...
   Hubbard_1D(const int system_size, const BoundaryCondition boundary_condition)
       : BaseU1Electron_1D<RealType>(system_size) {
   ...
   //! @param boundary_condition Boundary condition.
   //! Open boundary condition (OBC), periodic boundary condition (PBC), or sine square deformation (SSD).
   Hubbard_1D(const int system_size, const int total_electron, const BoundaryCondition boundary_condition)
       : BaseU1Electron_1D<RealType>(system_size, total_electron) {
      SetBoundaryCondition(boundary_condition);
      onsite_operator_ham_ = CreateOnsiteOperatorHam(h_z_);
   }
   ...
   //! \f[ \hat{H}_{\rm
   //! onsite}=h_z\hat{s}^{z}+U\hat{n}_{\uparrow}\hat{n}_{\downarrow}+\mu(\hat{n}_{\uparrow}+\hat{n}_{\downarrow})\f]
