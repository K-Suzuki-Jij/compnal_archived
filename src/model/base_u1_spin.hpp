...
class BaseU1Spin {
  ...
  public:
   ...
   BaseU1Spin() { SetOnsiteOperator(); }

   //! @brief Constructor of BaseU1Spin class.
   //! @param magnitude_spin The magnitude of the spin \f$ S \f$.
   explicit BaseU1Spin(const HalfInt magnitude_spin) { SetMagnitudeSpin(magnitude_spin); }
   ...
   QType CalculateQNumber(const IntegerType row, const IntegerType col, const QType &total_sz) const {
      ...
      return static_cast<QType>(col - row + total_sz);
   }
   ...
   std::vector<std::int64_t> GenerateBasis(const int system_size, const QType total_sz,
      ...
      if (!ValidateQNumber(system_size, magnitude_spin_, total_sz)) {
         ...
         ss << ") or magnitude_spin(" << magnitude_spin_ << ") or total_sz(" << total_sz << ")" << std::endl;
         throw std::runtime_error(ss.str());
      }
      ...
      auto partition_integers =
          utility::GenerateIntegerPartition(static_cast<int>(system_size * magnitude_spin_ - total_sz),
                                            static_cast<int>(2 * magnitude_spin_), system_size);
