...
class BaseU1SpinElectron {
  ...
  public:
   ...
   BaseU1SpinElectron() { SetOnsiteOperator(); }
   ...
   BaseU1SpinElectron(const HalfInt magnitude_lspin, const int total_electron, const HalfInt total_sz) {
      SetTotalElectron(total_electron);
      SetTotalSz(total_sz);
      SetMagnitudeLSpin(magnitude_lspin);
   }
         ...
         for (int n_up = 0; n_up <= total_electron - 2 * n_up_down; ++n_up) {
            ...
            if (0 <= n_up && 0 <= n_down && 0 <= n_vac) {
               ...
               if (BaseU1Spin<RealType>::ValidateQNumber(system_size, magnitude_lspin_, total_sz - total_sz_electron)) {
                  ...
                  const std::int64_t dim_electron = binom[system_size][n_up] * binom[system_size - n_up][n_down] *
                                                    binom[system_size - n_up - n_down][n_up_down];
                  const std::int64_t dim_lspin = BaseU1Spin<RealType>::CalculateTargetDim(system_size, magnitude_lspin_,
                                                                                          total_sz - total_sz_electron);
...
#pragma omp parallel for
      for (std::size_t i = 0; i < temp_q_number_spin_vec.size(); ++i) {
         ...
         const std::int64_t dim_target_lspin =
             BaseU1Spin<RealType>::CalculateTargetDim(system_size, magnitude_lspin_, total_2sz_lspin / 2);
         std::vector<std::vector<int>> partition_integers =
             utility::GenerateIntegerPartition(shifted_2sz, 2 * magnitude_lspin_);
...
#pragma omp parallel for
   ...
   //! @return ture if there exists corresponding subspace, otherwise false.
   static bool ValidateQNumber(const int system_size, const HalfInt magnitude_lspin, const int total_electron,
   ...
   static std::int64_t CalculateTargetDim(const int system_size, const HalfInt magnitude_lspin,
                                          const int total_electron, const HalfInt total_sz) {
         ...
         for (int n_up = 0; n_up <= total_electron - 2 * n_up_down; ++n_up) {
            ...
            if (0 <= n_up && 0 <= n_down && 0 <= n_vac) {
               ...
               const std::int64_t dim_lspin =
                   BaseU1Spin<RealType>::CalculateTargetDim(system_size, magnitude_lspin, total_sz - total_sz_electron);
   ...
   static CRS CreateOnsiteOperatorSpL(const HalfInt magnitude_lspin) {
      ...
      for (int row_c = 0; row_c < dim_onsite_electron; ++row_c) {
         for (int row_l = 1; row_l < dim_onsite_lspin; ++row_l) {
            matrix.val.push_back(
                std::sqrt(static_cast<RealType>((magnitude_lspin + 1) * 2 * row_l - row_l * (row_l + 1))));
            matrix.col.push_back(row_l + row_c * dim_onsite_lspin);
   ...
   static CRS CreateOnsiteOperatorSxL(const HalfInt magnitude_lspin) {
      ...
      for (int row_c = 0; row_c < dim_onsite_electron; ++row_c) {
         ...
         matrix.val.push_back(
             RealType{0.5} * std::sqrt(static_cast<RealType>((magnitude_lspin + 1) * (a + b + 1) - (a + 1) * (b + 1))));
         ...
         for (int row_l = 1; row_l < dim_onsite_lspin - 1; ++row_l) {
            a = row_l;
            b = row_l - 1;
            matrix.val.push_back(RealType{0.5} * std::sqrt(static_cast<RealType>((magnitude_lspin + 1) * (a + b + 1) -
                                                                                 (a + 1) * (b + 1))));
            matrix.col.push_back(b + row_c * dim_onsite_lspin);
   ...
   //! @brief Generate the spin-\f$ S\f$ operator of the local spin for the y-direction \f$ i\hat{S}^{y}\f$ with \f$
   //! i\f$ being the imaginary unit.
   //! @param magnitude_lspin The magnitude of the local spin \f$ S \f$.
  ...
  protected:
   ...
   inline int CalculateBasisOnsiteElectron(const int basis_onsite) const { return basis_onsite / dim_onsite_lspin_; }
