...
//! + h_z\sum^{N}_{i=1}\left(\hat{s}^{x}_{i} + \hat{S}^{x}_{i}\right) + D_z\sum^{N}_{i=1}\left(\hat{S}^{z}_{i}\right)^2
//! \f]
...
class KondoLattice_1D : public BaseU1SpinElectron_1D<RealType> {
  ...
  public:
   ...
   KondoLattice_1D(const int system_size, const double magnitude_lspin)
   ...
   KondoLattice_1D(const int system_size, const int total_electron)
   ...
   //! @param total_electron The number of the total electrons
   //! \f$ \langle \hat{N}_{e}\rangle =\sum^{N}_{i=1}\langle\hat{n}_{i}\rangle\f$.
   KondoLattice_1D(const int system_size, const double magnitude_lspin, const int total_electron)
   ...
   KondoLattice_1D(const int system_size, const BoundaryCondition boundary_condition)
   ...
   //! @param magnitude_lspin The magnitude of the local spin \f$ S \f$.
   //! @param boundary_condition Boundary condition.
   KondoLattice_1D(const int system_size, const double magnitude_lspin, const BoundaryCondition boundary_condition)
       : BaseU1SpinElectron_1D<RealType>(system_size, magnitude_lspin) {
      onsite_operator_ham_ = CreateOnsiteOperatorHam(0.5 * this->magnitude_2lspin_, J_z_, J_xy_, h_z_, D_z_);
      SetBoundaryCondition(boundary_condition);
   }
   ...
   //! \f$ \langle \hat{N}_{e}\rangle =\sum^{N}_{i=1}\langle\hat{n}_{i}\rangle\f$.
   //! @param boundary_condition Boundary condition.
   KondoLattice_1D(const int system_size, const int total_electron, const BoundaryCondition boundary_condition)
       : BaseU1SpinElectron_1D<RealType>(system_size, total_electron) {
      onsite_operator_ham_ = CreateOnsiteOperatorHam(0.5 * this->magnitude_2lspin_, J_z_, J_xy_, h_z_, D_z_);
      SetBoundaryCondition(boundary_condition);
   }
   ...
   static CRS CreateOnsiteOperatorHam(const double magnitude_lspin, const RealType J_z, const RealType J_xy,
                                      const RealType h_z, const RealType D_z) {
   ...
   //! @param D_z The uniaxial anisotropy to the z-direction \f$ D_z\f$.
   //! @return The matrix of \f$ \hat{H}_{\rm onsite}\f$.
   static CRS CreateOnsiteOperatorHam(const double magnitude_lspin, const RealType J, const RealType h_z,
                                      const RealType D_z) {
