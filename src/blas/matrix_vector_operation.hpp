...
void CalculateMatrixVectorProduct(BraketVector<ReturnType> *vector_out,
                                  const T1 coeff,
...
//! @brief Calculate matrix vector product.
//! \f$ \boldsymbol{v}_{\rm out} = c\hat{M}\cdot\boldsymbol{v}\f$.
//! Note that the matrix \f$ \hat{M}\f$ must be symmetric and their elements are stored only in lower triangle elements.
//! In addition, the diagonal elements must also stored even if they are zero.
//! @tparam ReturnType The value type of output vector.
//! @tparam T1 The value type of the coefficient \f$ c\f$.
//! @tparam T2 The value type of the matirx \f$ \hat{M}\f$.
//! @tparam T3 The value type of the vector \f$ \boldsymbol{v}\f$.
//! @param vector_out The pointer of result for matrix vector product \f$ \boldsymbol{v}_{\rm out} = c\hat{M}\cdot\boldsymbol{v}\f$.
//! @param coeff The coefficient \f$ c\f$.
//! @param matrix_in The matrix \f$ \hat{M} \f$.
//! @param vector_in The vector \f$ \boldsymbol{v} \f$.
//! @param vectors_work The pointer of a temporary two-dimensional array used for calculations. Their elements must be zero.
//! Note that this working array is used only when openmp is active.
template<typename ReturnType, typename T1, typename T2, typename T3>
void CalculateSymmetricMatrixVectorProduct(BraketVector<ReturnType> *vector_out,
                                           const T1 coeff,
                                           const CRS<T2> &matrix_in,
                                           const BraketVector<T3> &vector_in,
...
//! @param matrix_in The matrix \f$ \hat{M} \f$.
//! @param vector_in The vector \f$ \boldsymbol{v} \f$.
//! @return The result of matrix vector product \f$ \boldsymbol{v}_{\rm out} = c\hat{M}\cdot\boldsymbol{v}\f$.
template<typename T1, typename T2, typename T3>
auto CalculateMatrixVectorProduct(const T1 coeff,
                                  const CRS<T2> &matrix_in,
                                  const BraketVector<T3> &vector_in) ->
...
//! @param vectors_work The pointer of a temporary two-dimensional array used for calculations. Their elements must be zero.
//! Note that this working array is used only when openmp is active.
//! @return The matrix vector product \f$ \boldsymbol{v}_{\rm out} = c\hat{M}\cdot\boldsymbol{v}\f$.
template<typename T1, typename T2, typename T3>
auto CalculateSymmetricMatrixVectorProduct(const T1 coeff,
                                           const CRS<T2> &matrix_in,
                                           const BraketVector<T3> &vector_in,
                                           std::vector<std::vector<decltype(std::declval<T2>()*std::declval<T3>())>> *vectors_work = nullptr) ->
BraketVector<decltype(std::declval<T1>()*std::declval<T2>()*std::declval<T3>())> {
