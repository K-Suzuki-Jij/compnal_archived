...
void InverseIteration(CRS<RealType> *matrix_in, BraketVector<RealType> *eigenvector, const RealType eigenvalue,
...
#ifdef _OPENMP
      vectors_work_pthreads =
          std::vector<std::vector<RealType>>(omp_get_max_threads(), std::vector<RealType>(matrix_in->row_dim));
#endif
   }
   ...
   if (params.cg.flag_use_initial_vec) {
      if (static_cast<std::int64_t>(eigenvector->value_list.size()) != matrix_in->row_dim) {
         std::stringstream ss;
         ss << "Error in " << __func__ << std::endl;
         ss << "The dimension of the initial vector is not equal to that "
               "of "
               "the input matrix."
            << std::endl;
         throw std::runtime_error(ss.str());
   ...
   for (int step = 0; step < params.max_step; ++step) {
      ...
      if (residual_error < params.acc) {
         ...
         if (flag_display_info) {
            ...
            std::cout << std::defaultfloat << std::setprecision(8) << "\rElapsed time of inverse iteration:" << time_sec
                      << "[sec]";
            std::cout << " (" << residual_error << ")" << std::flush;
            std::cout << std::endl;
         }
      ...
      ConjugateGradient(&improved_eigenvector, *matrix_in, *eigenvector, subspace_vectors, flag_display_info,
                        params.cg);
