...
class BraketVector {
  public:
   ...
   BraketVector(const int64_t size) { value_list.resize(size); }
...
#pragma omp parallel for
   ...
   void Free() { std::vector<ElementType>().swap(this->value_list); }
...
#pragma omp parallel for
   ...
   //! @tparam T Value type.
   //! @param normalization_factor Normalized as \f$\|\boldsymbol{v}\|={\rm
   //! normalization\_factor} \f$.
   template <typename T = ElementType>
...
#pragma omp parallel for
   ...
   //! @return BraketVector object, \f$ -\boldsymbol{v} \f$.
   BraketVector operator-() const { return CalculateScalarVectorProduct(-1, *this); }

   //! @brief Operator overloading: compound assignment plus operator.
   //! @tparam T Value type of the right-hand side BraketVector.
   //! @param rhs The BraketVector of the right-hand side, \f$
   //! \boldsymbol{v}_{\rm rhs} \f$.
   //! @return BraketVector object, \f$ \boldsymbol{v} + \boldsymbol{v}_{\rm
   //! rhs}\f$.
   template <typename T>
...
//! @param lhs The BraketVector of the left-hand side, \f$ \boldsymbol{v}_{\rm
//! lhs} \f$.
//! @param rhs The BraketVector of the right-hand side, \f$ \boldsymbol{v}_{\rm
//! rhs} \f$.
//! @return BraketVector object, \f$ \boldsymbol{v}_{\rm lhs} +
//! \boldsymbol{v}_{\rm rhs}\f$.
...
//! @param rhs The BraketVector of the right-hand side, \f$ \boldsymbol{v}_{\rm
//! rhs} \f$.
//! @return Inner product, \f$ \boldsymbol{v}_{\rm lhs}\cdot\boldsymbol{v}_{\rm
//! rhs}\f$.
template <typename T1, typename T2>
auto operator*(const BraketVector<T1> &lhs, const BraketVector<T2> &rhs)
...
//! @return BraketVector object, \f$ \boldsymbol{v}_{\rm lhs}c_{\rm rhs} =
//! c_{\rm rhs}\boldsymbol{v}_{\rm lhs}\f$.
template <typename T1, typename T2>
...
//! @param rhs The BraketVector of the right-hand side, \f$ \boldsymbol{v}_{\rm
//! rhs} \f$.
//! @return Return true if \f$ \boldsymbol{v}_{\rm lhs}=\boldsymbol{v}_{\rm
//! rhs}\f$, otherwise false.
template <typename T1, typename T2>
...
//------------------------Global Functions--------------------------
//------------------------------------------------------------------
//! @brief Calculate BraketVector summation, \f$ c_1\boldsymbol{v}_1 +
//! c_2\boldsymbol{v}_2 \f$.
...
//! @return BraketVector object, \f$ c_1\boldsymbol{v}_{1} +
//! c_2\boldsymbol{v}_{2}\f$.
template <typename T1, typename T2, typename T3, typename T4>
auto CalculateVectorVectorSum(const T1 coeff_1, const BraketVector<T2> &braket_vector_1, const T3 coeff_2,
...
//! @brief Calculate BraketVector summation, \f$ c_1\boldsymbol{v}_1 +
//! c_2\boldsymbol{v}_2 \f$.
//! @tparam T1 Value type of output vector, \f$ c_1\boldsymbol{v}_{1} +
//! c_2\boldsymbol{v}_{2}\f$.
...
void CalculateVectorVectorSum(BraketVector<T1> *braket_vector_out, const T2 coeff_1,
                              const BraketVector<T3> &braket_vector_1, const T4 coeff_2,
                              const BraketVector<T5> &braket_vector_2) {
   if (braket_vector_1.value_list.size() != braket_vector_2.value_list.size()) {
      std::stringstream ss;
      ss << "Error at " << __LINE__ << " in " << __func__ << " in " << __FILE__ << std::endl;
      ss << "BraketVector types do not match each other" << std::endl;
      ss << "dim_1 = " << braket_vector_1.value_list.size() << ", dim_2 = " << braket_vector_2.value_list.size()
         << std::endl;
      throw std::runtime_error(ss.str());
   }
...
#pragma omp parallel for
   for (std::int64_t i = 0; i < size; ++i) {
      braket_vector_out->value_list[i] =
          coeff_1 * braket_vector_1.value_list[i] + coeff_2 * braket_vector_2.value_list[i];
   }
}

//! @brief Calculate BraketVector innner product, \f$
//! \boldsymbol{v}_1\cdot\boldsymbol{v}_2 \f$.
...
template <typename T1, typename T2>
auto CalculateVectorVectorProduct(const BraketVector<T1> &braket_vector_1, const BraketVector<T2> &braket_vector_2)
    -> decltype(std::declval<T1>() * std::declval<T2>()) {
   if (braket_vector_1.value_list.size() != braket_vector_2.value_list.size()) {
      std::stringstream ss;
      ss << "Error at " << __LINE__ << " in " << __func__ << " in " << __FILE__ << std::endl;
...
template <typename T1, typename T2>
auto CalculateScalarVectorProduct(const T1 value, const BraketVector<T2> &braket_vector)
    -> BraketVector<decltype(std::declval<T1>() * std::declval<T2>())> {
   using T1T2 = decltype(std::declval<T1>() * std::declval<T2>());
...
//! \f$ \|c_1\boldsymbol{v}_1 - c_2\boldsymbol{v}_2\|_1=\sum_{i}\|c_1v_{1,i} -
//! c_2v_{2,i}\| \f$.
//! @tparam T1 Value type of coeff_1.
//! @tparam T2 Value type of braket_vector_1.
//! @tparam T3 Value type of coeff_2.
...
template <typename T1, typename T2, typename T3, typename T4>
auto CalculateL1Distance(const T1 coeff_1, const BraketVector<T2> &braket_vector_1, const T3 coeff_2,
                         const BraketVector<T4> &braket_vector_2)
...
//! @brief Calculate L2 distance, aka Euclidean distance, \f$
//! \|c_1\boldsymbol{v}_1 - c_2\boldsymbol{v}_2\| \f$.
//! @tparam T1 Value type of coeff_1.
//! @tparam T2 Value type of braket_vector_1.
//! @tparam T3 Value type of coeff_2.
//! @tparam T4 Value type of braket_vector_2.
...
template <typename T1, typename T2, typename T3, typename T4>
auto CalculateL2Distance(const T1 coeff_1, const BraketVector<T2> &braket_vector_1, const T3 coeff_2,
                         const BraketVector<T4> &braket_vector_2)
    -> decltype(std::declval<T1>() * std::declval<T2>() - std::declval<T3>() * std::declval<T4>()) {
   if (braket_vector_1.value_list.size() != braket_vector_2.value_list.size()) {
      std::stringstream ss;
