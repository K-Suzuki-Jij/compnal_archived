...
extern "C" {
void dspgv_(const int &ITYPE, const char &JOBZ, const char &UPLO, const int &N, double *AP, double *BP, double *W,
            double **Z, const int &LDZ, double *WORK, int &INFO);
};

template <typename RealType>
void LapackDsyev(RealType *gs_value, std::vector<RealType> *gs_vector, const CRS<RealType> &matrix_in,
   ...
   dsyev_('V', 'L', static_cast<int>(matrix_in.row_dim), (double **)matrix_array, static_cast<int>(matrix_in.row_dim),
          val_array, work, static_cast<int>(3 * matrix_in.row_dim), info);
...
template <typename RealType>
void LapackDstev(RealType *gs_value, std::vector<RealType> *gs_vector, const std::vector<RealType> &diag,
...
template <typename RealType>
void LapackDspgv(std::vector<RealType> *eigenvalues, std::vector<RealType> *eigenvectors, const int i_type,
                 const int dim, const std::vector<RealType> &mat_a, const std::vector<RealType> &mat_b) {
