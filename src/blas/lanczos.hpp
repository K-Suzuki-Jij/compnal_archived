...
void EigendecompositionLanczos(RealType *target_value_out, BraketVector<RealType> *target_vector_out,
...
#pragma omp parallel for
      for (std::int64_t i = 0; i < dim; ++i) {
         vector_1.value_list[i] -=
             diagonal_value[step + 1] * vector_2.value_list[i] + off_diagonal_value[step] * vector_0.value_list[i];
...
#pragma omp parallel for
         for (std::int64_t i = 0; i < dim; ++i) {
            vector_1.value_list[i] -=
                diagonal_value[step] * vector_2.value_list[i] + off_diagonal_value[step - 1] * vector_0.value_list[i];
            vector_0.value_list[i] = vector_2.value_list[i];
         }
         if (flag_display_info) {
            std::cout << "\rLanczos_Vec_Step:" << step << "/" << converge_step_number << std::string(5, ' ')
                      << std::flush;
         }
      }
      target_vector_out->Normalize();
   ...
   if (flag_display_info) {
      const auto time_count =
          std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::system_clock::now() - start).count();
      const double time_sec = static_cast<double>(time_count) / TIME_UNIT_CONSTANT;
      std::cout << std::defaultfloat << std::setprecision(8) << "\rElapsed time of diagonalization:" << time_sec
                << "[sec]" << std::flush;
